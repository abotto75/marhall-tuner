...
export default async function handler(req, res) {
  try {
    ...
    const sys = `Sei un tecnico Marshall per Acton III. NON creare settaggi da zero: parti dal preset dato.
Mantieni i valori entro ±${delta.toFixed(1)} ore (limiti assoluti 0.5–3.5). Restituisci SOLO JSON (schema).
Aggiungi note pratiche (posizionamento vs parete/angolo, volume, warning su sub-bass).`;

    const proPrompt = `Contesto: ambiente="${room}", volume=${volume}%.
Preset di base fornito: bass_clock=${baseBass}, treble_clock=${baseTreble}.
Richiesta: ${query}
Istruzioni: se servono micro-ritocchi, limita gli spostamenti a ±${delta.toFixed(1)} ore. Non superare i limiti.`;
